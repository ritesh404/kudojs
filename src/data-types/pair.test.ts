...
test("Pair", t => {
    ...
    t.deepEqual(compose(unwrap, fmap(id))(a), compose(unwrap)(a), "should pass the identity law");
    ...
    t.throws(() => a.ap(Pair(1,1)), "applicative expects second to be a function type")
    t.throws(() => Pair(1,1).ap(Pair(1,add2)), "applicative expects both values to be semigroups")
    t.ok(a.ap(c.ap(b.map(p => q => x => p(q(x))))).toString() === a.ap(c).ap(b).toString(), "should pass applicative composition")
    ...
    t.ok(Pair(1,2).swap().equals(Pair(2,1)), "swap");
    ...
    t.throws(() => a.concat(Pair({a:"1"}, {a: "1"})), "concat expects a Pair with semigroups");
    t.ok(a.concat(Pair([2],[2])).toString() === Pair([1,2],[2,2]).toString(), "concat")

    t.throws(() => a.chain(1), "chain expects a function")
    t.ok(
        gimmePair([1]).chain(gimmePair).chain(gimmePair).toString() === gimmePair([1]).chain(x => gimmePair(x).chain(gimmePair)).toString()
    , "chain Associativity");
