...
test("Task#Functor Composition", t => {
    let res1 = "res1";
    let res2 = "res2";
    a.map(x => sub1(add2(x))).fork(id, x => {res1 = x})
    a.map(sub1).map(add2).fork(id, x => {res2 = x});
    setTimeout(() => {
...
test("Task#Applicative", t => {
    ...
    a.ap(c.ap(b.map(p => q => x => p(q(x)))))
    .fork(id, x => {
        res1 =x;
    })
    a.ap(c).ap(b).fork(id, x => {
        res2 =x;
    })
    setTimeout(() => {
...
test("Task#Chain Associativity", t => {
    ...
    gimmeTask(1).chain(gimmeTask).chain(gimmeTask).fork(id, x => {
        res1 = x+1;
    });
    gimmeTask(1).chain(x => gimmeTask(x).chain(gimmeTask)).fork(id, x => {
        res2 = x+1;
    });
    setTimeout(() => {
