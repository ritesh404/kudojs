...
const prop = (k: any) => (xs: Object) => k in xs ? Either.Right(xs[k]) : Either.Left(1)
...
test("Either",(t) => {
    ...
    t.equals(compose(unwrap, fmap(id))(a), compose(unwrap)(a), "Right should pass the identity law");
    t.equals(compose(unwrap, fmap(id))(e), compose(unwrap)(e), "Left should pass the identity law");
    ...
    t.ok(a.ap(c.ap(b.map(p => q => x => p(q(x))))).equals(a.ap(c).ap(b)), "Right should pass applicative composition")
    
    t.equals(unwrap(a.map(add2)), unwrap(a.ap(b)), "Right mapped to a function should the same as the function applied to Right");
    t.equals(unwrap(e.map(add2)), unwrap(e.ap(b)), "Left mapped to a function should the same as the function applied to Left");
    
    t.equals(unwrap(Either.of(6)), unwrap(Either.Right(6)), "of creates a Right");

    t.equals(unwrap(Either.fromNullable(1)), unwrap(a), "creates a Right when non nullable value is passed");
    t.equals(unwrap(Either.fromNullable(null)), unwrap(Either.Left(null)), "creates a Left when nullable value is passed");

    t.equals(unwrap(Either.withDefault(1, 2)), unwrap(Either.Right(2)), "creates a Right with default value if nullable value is passed");
    t.equals(unwrap(Either.withDefault(1, null)), unwrap(a), "creates a Right with default value if nullable value is passed");

    t.equals(unwrap(Either.swap(a)), unwrap(Either.Left(1)), "swaps right to left");
    t.equals(unwrap(Either.swap(e)), unwrap(Either.Right(1)), "swaps left to right");

    t.equals(unwrap(Either.try(x => {
        return x + 6;
    })(1)), unwrap(Either.Right(7)), "Right on succesfull try");
    t.equals(unwrap(Either.try(() => {
        throw "error";
    })()), unwrap(Either.Left("error")), "Left on failed try");

    t.equals(unwrap(Either.bimap(a, v => v+1, v => v+2)), unwrap(Either.Right(3)), "bimap Right");
    t.equals(unwrap(Either.bimap(e, v => v+1, v => v+2)), unwrap(Either.Left(2)), "bimap Left");
    ...
    t.ok(Either.Right(1).equals(a) === a.equals(Either.Right(1)),"Right commutativity");
    ...
    t.ok(Either.Left(1).equals(e) === e.equals(Either.Left(1)),"Left commutativity");
    ...
    t.throws(() => caseOf({
        Left: id,
    }, a), "Either: Expected Right");
    t.throws(() => caseOf({
        Right: id,
    }, e), "Either: Expected Left");
